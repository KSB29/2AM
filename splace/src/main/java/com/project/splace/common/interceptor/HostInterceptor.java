...
public class HostInterceptor extends HandlerInterceptorAdapter {
	...
	private Logger logger = LoggerFactory.getLogger(HostInterceptor.class);
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if(!url.equals("/hostApplyForm.sp")) {
			if(!grade.equals("2")) {
				logger.info("등급이 호스트가 아닌 상태에서 [" + request.getRequestURI() + "] 접근");
				
				session.setAttribute("msg", "호스트신청 후 이용하세요");
				response.sendRedirect("hostApplyForm.sp");
